    ...
    <el-table v-loading="loading" :data="list" border fit highlight-current-row style="width: 100%">
      <el-table-column align="center" label="Group">
        <template slot-scope="scope">
          <span>{{ scope.row.ending_frame_group ? scope.row.ending_frame_group : 'Unknown' }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="Requests">
        <template slot-scope="scope">
          <span>{{ scope.row.request_count ? scope.row.request_count : 0 }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="Impressions">
        <template slot-scope="scope">
          <span>{{ scope.row.impression_count ? scope.row.impression_count : 0 }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="Clicks">
        <template slot-scope="scope">
          <span>{{ scope.row.click_count ? scope.row.click_count : 0 }}</span>
        </template>
      </el-table-column>
      ...
      <!--<template>-->
      <!--<router-link class="link-type" :to="'/statis/group/channel'">Channels</router-link>-->
      <!--</template>-->
      <!--</el-table-column>-->
    </el-table>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      this.loading = true;
      const { data, meta } = await statis.group(this.query);
      this.list = data;
