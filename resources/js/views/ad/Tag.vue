<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-permission="['advertise.campaign.ad.edit']" class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-plus" @click="handleCreate">
        {{ $t('table.add') }}
      </el-button>
    ...
    >
      ...
      <!-- <el-table-column align="center" label="Actions" width="360">
        <template slot-scope="scope">
          <el-button v-permission="['advertise.campaign.ad.edit']" type="primary" size="small" icon="el-icon-edit" @click="handleEdit(scope.row)">
            Edit
          </el-button>
        </template>
      </el-table-column> -->
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      this.loading = true;
      const { data, meta } = await campaignResource.tagList(this.query);
      this.list = data;
    ...
    handleStatus(account) {
      if (!checkPermission(['advertise.campaign.ad.edit'])) {
        this.$message({
          type: 'warning',
      ...
      this.$confirm('This will ' + (account.status ? 'disable' : 'enable') + ' account ' + displayName + '. Continue?', 'Warning', {
        ...
        if (account.status) {
          campaignResource.disable(account.id).then(response => {
            this.$message({
              type: 'success',
          ...
          campaignResource.enable(account.id).then(response => {
            this.$message({
              type: 'success',
